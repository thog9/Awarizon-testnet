...
░░░╚═╝░░░╚═╝░░╚═╝░╚════╝░░╚═════╝░░░░╚═╝░░░░╚════╝░░╚════╝░╚══════╝╚═╝░░╚═╝░╚═════╝░╚═════╝░
    ...
    print_border("SWARMBASE AI", Fore.GREEN)
...
async def run_node(language):
    from scripts.node import run_node
    await run_node(language)

async def run_connect(language):
...
def get_available_scripts(language):
    scripts = {
        'vi': [
            {"name": "1. Tự động kích hoạt & Xem Node", "value": "node"},
            {"name": "2. Tự động Checkin hàng ngày", "value": "checkin"},
            {"name": "3. Kết nối Social ( Twitter | Telegram | Discord )", "value": "connect"},
          
            {"name": "X. Thoát", "value": "exit"},
        ],
        'en': [
            {"name": "1. Automatic Activate & View Node", "value": "node"},
            {"name": "2. Automatically Daily Check-in", "value": "checkin"},
            {"name": "3. Connect Social ( Twitter | Telegram | Discord )", "value": "connect"},
